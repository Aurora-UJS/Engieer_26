...
void MahonyAHRSinit(float ax, float ay, float az, float mx, float my, float mz)
{
	...
	arm_atan2_f32(-ax, az, &init_pitch);
	arm_atan2_f32(-ay, az, &init_roll);

	sin_roll = arm_sin_f32(init_roll);
	cos_roll = arm_cos_f32(init_roll);
	cos_pitch = arm_cos_f32(init_pitch);
	sin_pitch = arm_sin_f32(init_pitch);
	...
	if ((mx != 0.0f) && (my != 0.0f) && (mz != 0.0f))
	{
		...
		arm_atan2_f32(-magY, magX, &init_yaw);
	}
	else
	{
		init_yaw = 0.0f;
	...
	cr2 = arm_cos_f32(init_roll * 0.5f);
	cp2 = arm_cos_f32(init_pitch * 0.5f);
	cy2 = arm_cos_f32(init_yaw * 0.5f);
	sr2 = arm_sin_f32(init_roll * 0.5f);
	sp2 = arm_sin_f32(init_pitch * 0.5f);
	sy2 = arm_sin_f32(init_yaw * 0.5f);
