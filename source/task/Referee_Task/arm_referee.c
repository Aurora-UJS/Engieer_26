...
void Arm_Keyboard_Manager(uint8_t key, const keyboard_t *kb) {

  if (key == (uint8_t)'Q') {
    if (kb != NULL && kb->key_code.bit.SHIFT != 0U) {
      Target_Point[0].target_joint_radian -= 0.1f;
    } else {
      Target_Point[0].target_joint_radian += 0.1f;
    }
  }
  if (key == (uint8_t)'E') {
    if (kb != NULL && kb->key_code.bit.SHIFT != 0U) {
      Target_Point[1].target_joint_radian -= 0.1f;
    } else {
      Target_Point[1].target_joint_radian += 0.1f;
    }
  }
  ...
  if (key == (uint8_t)'Z') {
    ...
    if (kb != NULL && kb->key_code.bit.SHIFT != 0U) {
      Target_Point[3].target_joint_radian -= 0.1f;
    } else {
      Target_Point[3].target_joint_radian += 0.1f;
    }
  }
  ...
  if (key == (uint8_t)'C') {
    if (kb != NULL && kb->key_code.bit.SHIFT != 0U) {
      Target_Point[5].target_joint_radian -= 0.1f;
    } else {
      Target_Point[5].target_joint_radian += 0.1f;
    }
  }
