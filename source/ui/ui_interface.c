//
// Created by bismarckkk on 2025/3/22.
// Dynamic Edition
//

#include <string.h>
#include <stdio.h>
#include "ui_interface.h"
#include "referee_protocol.h"

#define UI_MAX_FRAMES_PER_CALL 1

static uint8_t ui_seq = 0U;
uint16_t ui_self_id = RobotID_REngineer;
static int s_ui_string_scan_cursor = 0;

static void ui_send_message(const uint8_t *message, uint16_t length);
static uint16_t ui_get_client_id(uint16_t robot_id);
static ui_send_hook_t s_ui_send_hook = print_message;

ui_string_frame_t _ui_string_frame;
ui_1_frame_t _ui_1_frame;
ui_2_frame_t _ui_2_frame;
ui_5_frame_t _ui_5_frame;
ui_7_frame_t _ui_7_frame;

void print_message(const uint8_t *message, uint16_t length) {
    for (int i = 0; i < length; i++) {
        printf("%02x ", message[i]);
    }
    printf("\n\n");
}

void ui_set_send_hook(ui_send_hook_t hook)
{
    s_ui_send_hook = (hook != NULL) ? hook : print_message;
}

static void ui_send_message(const uint8_t *message, uint16_t length)
{
    if (s_ui_send_hook != NULL) {
        s_ui_send_hook(message, length);
    }
}

static uint16_t ui_get_client_id(uint16_t robot_id)
{
    return (uint16_t)(robot_id + 0x0100U);
}

static const unsigned char CRC8_TAB[256] = {
    ,
};

unsigned char calc_crc8(unsigned char *pchMessage, unsigned int dwLength) {
    unsigned char ucCRC8 = 0xff;
    unsigned char ucIndex;
    while (dwLength--) {
        ucIndex = ucCRC8 ^ (*pchMessage++);
        ucCRC8 = CRC8_TAB[ucIndex];
    }
    return (ucCRC8);
}

static const uint16_t wCRC_Table[256] = {
    0x0000, 0x1189, 0x2312, 0x329b, 0x4624, 0x57ad, 0x6536, 0x74bf,
    0x8c48, 0x9dc1, 0xaf5a, 0xbed3, 0xca6c, 0xdbe5, 0xe97e, 0xf8f7,
    0x1081, 0x0108, 0x3393, 0x221a, 0x56a5, 0x472c, 0x75b7, 0x643e,
    0x9cc9, 0x8d40, 0xbfdb, 0xae52, 0xdaed, 0xcb64, 0xf9ff, 0xe876,
    0x2102, 0x308b, 0x0210, 0x1399, 0x6726, 0x76af, 0x4434, 0x55bd,
    0xad4a, 0xbcc3, 0x8e58, 0x9fd1, 0xeb6e, 0xfae7, 0xc87c, 0xd9f5,
    0x3183, 0x200a, 0x1291, 0x0318, 0x77a7, 0x662e, 0x54b5, 0x453c,
    0xbdcb, 0xac42, 0x9ed9, 0x8f50, 0xfbef, 0xea66, 0xd8fd, 0xc974,
    0x4204, 0x538d, 0x6116, 0x709f, 0x0420, 0x15a9, 0x2732, 0x36bb,
    0xce4c, 0xdfc5, 0xed5e, 0xfcd7, 0x8868, 0x99e1, 0xab7a, 0xbaf3,
    0x5285, 0x430c, 0x7197, 0x601e, 0x14a1, 0x0528, 0x37b3, 0x263a,
    0xdecd, 0xcf44, 0xfddf, 0xec56, 0x98e9, 0x8960, 0xbbfb, 0xaa72,
    0x6306, 0x728f, 0x4014, 0x519d, 0x2522, 0x34ab, 0x0630, 0x17b9,
    0xef4e, 0xfec7, 0xcc5c, 0xddd5, 0xa96a, 0xb8e3, 0x8a78, 0x9bf1,
    0x7387, 0x620e, 0x5095, 0x411c, 0x35a3, 0x242a, 0x16b1, 0x0738,
    0xffcf, 0xee46, 0xdcdd, 0xcd54, 0xb9eb, 0xa862, 0x9af9, 0x8b70,
    0x8408, 0x9581, 0xa71a, 0xb693, 0xc22c, 0xd3a5, 0xe13e, 0xf0b7,
    0x0840, 0x19c9, 0x2b52, 0x3adb, 0x4e64, 0x5fed, 0x6d76, 0x7cff,
    0x9489, 0x8500, 0xb79b, 0xa612, 0xd2ad, 0xc324, 0xf1bf, 0xe036,
    0x18c1, 0x0948, 0x3bd3, 0x2a5a, 0x5ee5, 0x4f6c, 0x7df7, 0x6c7e,
    0xa50a, 0xb483, 0x8618, 0x9791, 0xe32e, 0xf2a7, 0xc03c, 0xd1b5,
    0x2942, 0x38cb, 0x0a50, 0x1bd9, 0x6f66, 0x7eef, 0x4c74, 0x5dfd,
    0xb58b, 0xa402, 0x9699, 0x8710, 0xf3af, 0xe226, 0xd0bd, 0xc134,
    0x39c3, 0x284a, 0x1ad1, 0x0b58, 0x7fe7, 0x6e6e, 0x5cf5, 0x4d7c,
    0xc60c, 0xd785, 0xe51e, 0xf497, 0x8028, 0x91a1, 0xa33a, 0xb2b3,
    0x4a44, 0x5bcd, 0x6956, 0x78df, 0x0c60, 0x1de9, 0x2f72, 0x3efb,
    0xd68d, 0xc704, 0xf59f, 0xe416, 0x90a9, 0x8120, 0xb3bb, 0xa232,
    0x5ac5, 0x4b4c, 0x79d7, 0x685e, 0x1ce1, 0x0d68, 0x3ff3, 0x2e7a,
    0xe70e, 0xf687, 0xc41c, 0xd595, 0xa12a, 0xb0a3, 0x8238, 0x93b1,
    0x6b46, 0x7acf, 0x4854, 0x59dd, 0x2d62, 0x3ceb, 0x0e70, 0x1ff9,
    0xf78f, 0xe606, 0xd49d, 0xc514, 0xb1ab, 0xa022, 0x92b9, 0x8330,
    0x7bc7, 0x6a4e, 0x58d5, 0x495c, 0x3de3, 0x2c6a, 0x1ef1, 0x0f78
};

uint16_t calc_crc16(uint8_t *pchMessage, uint32_t dwLength)
{
    uint16_t wCRC = 0xffff;
    uint8_t chData;
    if (pchMessage == NULL)
    {
        return 0xFFFF;
    }
    while(dwLength--)
    {
        chData = *pchMessage++;
        (wCRC) = ((uint16_t)(wCRC) >> 8) ^ wCRC_Table[((uint16_t)(wCRC) ^ (uint16_t)(chData)) & 0x00ff];
    }
    return wCRC;
}

#define DEFINE_FRAME_PROC(num, id)                          \
void ui_proc_ ## num##_frame(ui_ ## num##_frame_t *msg) {   \
    msg->header.SOF = 0xA5;                                 \
    msg->header.length = Interactive_Data_LEN_Head + UI_Operate_LEN_PerDraw * num; \
    msg->header.seq = ui_seq++;                             \
    msg->header.crc8 = calc_crc8((uint8_t*)msg, 4);        \
    msg->header.cmd_id = ID_student_interactive;            \
    msg->header.sub_id = id;                                \
    msg->header.send_id = ui_self_id;                       \
    msg->header.recv_id = ui_get_client_id(ui_self_id);     \
    msg->crc16 = calc_crc16((uint8_t*)msg, (uint32_t)(sizeof(*msg) - sizeof(msg->crc16))); \
}

DEFINE_FRAME_PROC(1, 0x0101)
DEFINE_FRAME_PROC(2, 0x0102)
DEFINE_FRAME_PROC(5, 0x0103)
DEFINE_FRAME_PROC(7, 0x0104)

void ui_proc_string_frame(ui_string_frame_t *msg) {
    msg->header.SOF = 0xA5;
    msg->header.length = Interactive_Data_LEN_Head + UI_Operate_LEN_DrawChar;
    msg->header.seq = ui_seq++;
    msg->header.crc8 = calc_crc8((uint8_t *) msg, 4);
    msg->header.cmd_id = ID_student_interactive;
    msg->header.sub_id = UI_Data_ID_DrawChar;
    msg->header.send_id = ui_self_id;
    msg->header.recv_id = ui_get_client_id(ui_self_id);
    msg->option.str_length = strlen(msg->option.string);
    msg->crc16 = calc_crc16((uint8_t *) msg, (uint32_t)(sizeof(*msg) - sizeof(msg->crc16)));
}

void ui_proc_delete_frame(ui_delete_frame_t *msg) {
    msg->header.SOF = 0xA5;
    msg->header.length = Interactive_Data_LEN_Head + UI_Operate_LEN_Del;
    msg->header.seq = ui_seq++;
    msg->header.crc8 = calc_crc8((uint8_t *) msg, 4);
    msg->header.cmd_id = ID_student_interactive;
    msg->header.sub_id = UI_Data_ID_Del;
    msg->header.send_id = ui_self_id;
    msg->header.recv_id = ui_get_client_id(ui_self_id);
    msg->crc16 = calc_crc16((uint8_t *) msg, (uint32_t)(sizeof(*msg) - sizeof(msg->crc16)));
}

ui_delete_frame_t ui_delete_frame;

void ui_delete_layer(const uint8_t delete_type, const uint8_t layer) {
    ui_delete_frame.delete_type = delete_type;
    ui_delete_frame.layer = layer;
    ui_proc_delete_frame(&ui_delete_frame);
    ui_send_message((uint8_t *) &ui_delete_frame, sizeof(ui_delete_frame));
}

void ui_scan_and_send(const ui_interface_figure_t *ui_now_figures, uint8_t *ui_dirty_figure,
                      const ui_interface_string_t *ui_now_strings, uint8_t *ui_dirty_string, 
                      const int total_figures, const int total_strings) {
    int frame_budget = UI_MAX_FRAMES_PER_CALL;

    if (total_figures > 0) {
        int total_figure = 0;
        for (int i = 0; i < total_figures; i++) {
            if (ui_dirty_figure[i] > 0) {
                total_figure++;
            }
        }
        for (int i = 0, now_cap = 0, pack_size = 0; (i < total_figures) && (frame_budget > 0); i++) {
            if (ui_dirty_figure[i] > 0) {
                const int now_idx = now_cap % 7;
                if (now_idx == 0) {
                    const int remain_size = total_figure - now_cap;
                    if (remain_size > 5) {
                        pack_size = 7;
                    } else if (remain_size > 2) {
                        pack_size = 5;
                    } else if (remain_size > 1) {
                        pack_size = 2;
                    } else {
                        pack_size = 1;
                    }
                }
                if (pack_size == 7) {
                    _ui_7_frame.data[now_idx] = ui_now_figures[i];
                } else if (pack_size == 5) {
                    _ui_5_frame.data[now_idx] = ui_now_figures[i];
                } else if (pack_size == 2) {
                    _ui_2_frame.data[now_idx] = ui_now_figures[i];
                } else {
                    _ui_1_frame.data[now_idx] = ui_now_figures[i];
                }
                if (now_idx + 1 == pack_size || now_cap + 1 == total_figure) {
                    for (int j = now_idx + 1; j < pack_size; j++) {
                        if (pack_size == 7) {
                            _ui_7_frame.data[j].operate_type = 0;
                        } else if (pack_size == 5) {
                            _ui_5_frame.data[j].operate_type = 0;
                        } else if (pack_size == 2) {
                            _ui_2_frame.data[j].operate_type = 0;
                        } else {
                            _ui_1_frame.data[j].operate_type = 0;
                        }
                    }
                    if (pack_size == 7) {
                        ui_proc_7_frame(&_ui_7_frame);
                        ui_send_message((uint8_t *) &_ui_7_frame, sizeof(_ui_7_frame));
                    } else if (pack_size == 5) {
                        ui_proc_5_frame(&_ui_5_frame);
                        ui_send_message((uint8_t *) &_ui_5_frame, sizeof(_ui_5_frame));
                    } else if (pack_size == 2) {
                        ui_proc_2_frame(&_ui_2_frame);
                        ui_send_message((uint8_t *) &_ui_2_frame, sizeof(_ui_2_frame));
                    } else {
                        ui_proc_1_frame(&_ui_1_frame);
                        ui_send_message((uint8_t *) &_ui_1_frame, sizeof(_ui_1_frame));
                    }
                    frame_budget--;
                }
                now_cap++;
                ui_dirty_figure[i]--;
            }
        }
    }
    if ((total_strings > 0) && (frame_budget > 0)) {
        for (int scanned = 0; (scanned < total_strings) && (frame_budget > 0); scanned++) {
            const int idx = (s_ui_string_scan_cursor + scanned) % total_strings;
            if (ui_dirty_string[idx] > 0) {
                _ui_string_frame.option = ui_now_strings[idx];
                ui_proc_string_frame(&_ui_string_frame);
                ui_send_message((uint8_t *) &_ui_string_frame, sizeof(_ui_string_frame));
                ui_dirty_string[idx]--;
                frame_budget--;
                s_ui_string_scan_cursor = (idx + 1) % total_strings;
            }
        }
    }
}
